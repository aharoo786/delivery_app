...
import '../../address/screens/give_access_location.dart';
...
class AuthProvider with ChangeNotifier {
  ...
  Future<ResponseModel> registration(BuildContext buildContext,
    ...
    if (apiResponse.response != null &&
      ...
      if (token != null) {
        ...
        print(
            '-----------------------(REGISTRATION SCREEN) is $type and $userInput');
        verificationProvider.sendVerificationCode(
            buildContext, config, userInput,
  ...
  Future<ResponseModel> login(BuildContext buildContext, String userInput,
    ...
    print(
        "-------------------------(LOGIN)-----------------UserInput : $userInput and Password: $password and Type: $type");

    ApiResponseModel apiResponse = await authRepo!
        .login(userInput: userInput, password: password, type: type);
    ResponseModel responseModel;
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      String? token;
      ...
      if (token != null) {
        ...
        print(
            "-----------------------(TEMP TOKEN) : User Input: $userInput , FromPage: $fromPage");
        await verificationProvider.sendVerificationCode(
            buildContext, splashProvider.configModel!, userInput,
            type: type, fromPage: fromPage);
      }
  ...
  Future<ResponseModel> resetPassword(String? userInput, String? resetToken,
    ...
    print(
        "------------------------------------(RESET PASSWORD)----------------UserInput: $userInput, Reset Token: $resetToken, Password: $password, ConfirmPassword: $confirmPassword and Type: $type");

    ApiResponseModel apiResponse = await authRepo!.resetPassword(
        userInput, resetToken, password, confirmPassword,
        type: type);
    _isLoading = false;
    notifyListeners();
    ResponseModel responseModel;
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      print(
          "--------------------------(RESET PASSWORD)------------${apiResponse.response?.data}");
  ...
  Future socialLogin(SocialLoginModel socialLogin, Function callback) async {
    ...
    if (apiResponse.response?.statusCode == 200 &&
      ...
      print(
          "-----------------------(SOCIAL LOGIN API)-----------${apiResponse.response?.data}");
      Map map = apiResponse.response?.data;
      String? message = '';
      String? token = '';
      String? tempToken = '';
      ...
      if (map.containsKey('user')) {
        try {
          ...
          print(
              "--------------(SOCIAL Email)--------------${socialLogin.email}");
          print(
              "--------------(SOCIAL Medium)--------------${socialLogin.medium}");
      ...
      if (tempToken != null) {
        ...
        callback(true, null, message, tempToken, null, null, socialLogin.email,
            socialLogin.name);
      }
      notifyListeners();
    } else {
  ...
  Future<void> firebaseOtpLogin(
    ...
    print(
        "---------------(FIREBASE OTP LOGIN)--------------$phoneNumber, $session, $otp, $isForgetPassword");
    ...
    if (apiResponse.response != null &&
      ...
      if (isForgetPassword) {
        ...
        if (token != null) {
          String? countryCode =
              PhoneNumberCheckerHelper.getCountryCode(phoneNumber);
  ...
  Future<ResponseModel> registerWithOtp(String name,
      {String? email, required String phone, String? referralCode}) async {
    _isLoading = true;
    _loginErrorMessage = '';
    notifyListeners();
    print(
        "----------------------(REGISTER WITH OTP)----------- Email : $email , Phone: $phone , Name $name and Referral Code: $referralCode");
    ApiResponseModel apiResponse = await authRepo!.registerWithOtp(name,
        email: email, phone: phone, referralCode: referralCode);
    ResponseModel responseModel;
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      print(
          "----------------------(REGISTER WITH OTP)----------- API RESPONSE: ${apiResponse.response?.data}");
      String? token;
      Map map = apiResponse.response!.data;
  ...
  Future<(ResponseModel, String?)> registerWithSocialMedia(String name,
      {required String email, String? phone, String? referralCode}) async {
    _isLoading = true;
    _loginErrorMessage = '';
    notifyListeners();
    ApiResponseModel apiResponse = await authRepo!.registerWithSocialMedia(name,
        email: email, phone: phone, referralCode: referralCode);
    ResponseModel responseModel;
    String? token;
    String? tempToken;
